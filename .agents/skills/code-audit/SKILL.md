...
Before treating a bug as new, check whether it is already listed in the contest known-issues trackers or public mid-contest fixes.
...
- state the attacker position and controlled inputs
- be reproducible without guesswork

Good PoC forms:
- crafted packet or packet sequence causing crash, corruption, or bank-hash mismatch in local cluster
- harness input demonstrating crash or bank-hash mismatch
- minimally modified validator or client for consensus or replay bugs, with a clear reason the modification models a mainnet-capable attacker
- sandbox-escape demonstration
