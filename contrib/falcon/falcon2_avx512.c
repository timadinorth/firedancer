/* falcon2_avx512.c - AVX-512 Falcon-512 verification, Shoup variant.
 *
 ...
 * fq multiplication used inside the NTTs and Hadamard differs. */

int
falcon2_avx512_crypto_sign_open( uint8_t       * m,  size_t * mlen,
                                 uint8_t const * sm, size_t   smlen,
                                 uint8_t const * pk ) {
  ...
  /* Hadamard: a*b mod Q.  Neither operand has a precomputed s', so we
     materialize one for b on the fly.  This is one extra 64-bit divide
     per element -- acceptable since it happens once per coefficient
     versus three NTT passes' worth of butterflies per element. */
  for( int i=0; i<N; i+=16 ) {
    __m512i a = _mm512_loadu_si512( (void const *)( s2_ntt + i ) );
    __m512i b = _mm512_loadu_si512( (void const *)( h_ntt  + i ) );
    /* Compute b' = floor(b * 2^32 / Q) per lane.  We do this with the
       scalar formula 16 lanes at a time using `_mm512_mul_epu32` and
       `_mm512_div_epu32`; but `_mm512_div_epu32` is implemented via
       `vpaddd` on Intel so we'd lose the Shoup advantage entirely.
       Use plain Barrett mul here -- just for the Hadamard, NOT inside
       the NTT.  Three NTT passes contain ~6912 butterflies, while the
       Hadamard is only 512 lane-multiplies. */
