...
| AVX-512             | `falcon_avx512_crypto_sign_open`         | `falcon_avx512.c` (the AVX-512 implementation described in the paper)                 |
...
- the three SHAKE256-based verifiers (`falcon_ref`, `falcon_ref_xkcp`,
  `falcon_avx512`) accept the valid test vector and recover the
  message byte-for-byte;
- the three SHAKE256-based verifiers reject a tampered message,
  signature, nonce and public key;
- `falcon_ref_turbopar` runs deterministically (it returns -1 on the
  standard test vector because its hash differs).
...
`bench.c`, `test_falcon.c`, `Makefile`) is public domain.  The
...
(Apache 2.0) in `src/ballet/falcon` and relicensed to public domain
for this directory.
